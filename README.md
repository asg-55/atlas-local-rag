...
Данные и модели остаются на вашем компьютере или сервере. Текущий стабильный способ запуска использует локальную [Ollama](https://ollama.com/) и Docker. Для будущей Windows-сборки без Docker/Ollama добавлен экспериментальный клиент `llama-server`; результаты аудита и план находятся в [docs/windows-portable-audit.md](docs/windows-portable-audit.md).
...
- отдельный лаконичный режим анализа XLSX, CSV и JSON со сводкой по всему набору данных;
- простой чат для небольших рабочих решений на VBA, Python и C# без выполнения кода;
...
Задача автоматизации -> полный блок VBA/Python/C# -> копирование пользователем
...
Для XLSX, CSV и JSON Atlas заранее определяет столбцы и число строк, считает пропуски, минимум, максимум, среднее и частые текстовые значения. Эта сводка охватывает весь файл и всегда сохраняется в контексте; модель использует её для выводов, а релевантные исходные строки — для детализации. Раздел `Анализ` включает этот сценарий без дополнительных форм и быстрых действий: достаточно прикрепить файл и описать задачу обычными словами.
...
Раздел `Код` предназначен для небольших повседневных задач: макросов VBA, Python-скриптов и компактных утилит на C#. Atlas возвращает полный самодостаточный код в стандартном Markdown-блоке со встроенной кнопкой копирования, затем кратко объясняет установку зависимостей, место вставки и запуск. Перед показом результата выполняется отдельный проход проверки. Если локально установлена `qwen2.5-coder:7b`, Atlas автоматически использует её для этого раздела; иначе остаётся на выбранной общей модели. Встроенного редактора, терминала и автоматического выполнения нет: это сохраняет локальную версию простой, предсказуемой и безопасной.
