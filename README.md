...
Вложения остаются доступны для следующих вопросов этого диалога и после перезапуска приложения. В режиме `Автоматически` Atlas сам решает, нужен ли дополнительно поиск в постоянной базе; режимы `Только документы` и `Без базы знаний` позволяют явно зафиксировать поведение. Удаление диалога удаляет и его вложения.
