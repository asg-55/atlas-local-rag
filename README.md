...
- обратимое обезличивание DOCX, XLSX, PPTX и Outlook EML с зашифрованным ключом;
...
Office/EML -> проверка найденных данных -> нейтральные метки + зашифрованный ключ
                                              |
...
| `OUTPUT_DIR` | `/app/outputs` | Каталог экспортов, включая результаты обезличивания |
...
Во вкладке `Обезличивание` Atlas работает отдельно от RAG и Ollama. Поддерживаются современные форматы Word (`DOCX`), Excel (`XLSX`), PowerPoint (`PPTX`) и сохраненные из Outlook письма (`EML`). Форматирование Office сохраняется: Atlas заменяет текст внутри исходного OOXML-пакета, а не пересобирает документ с нуля. В EML обрабатываются заголовки, текст письма и вложенные DOCX/XLSX/PPTX.
...
Для обратного преобразования нужны обезличенный документ, соответствующий ключ и пароль. Пароль не хранится, поэтому его утрата делает восстановление невозможным. Результаты сохраняются в `outputs/anonymized/<указанная папка>` и также доступны для скачивания из браузера. В Docker каталог `outputs/` подключен с компьютера как отдельный том.
...
Для производственных XLSX-выгрузок Atlas автоматически распознаёт обозначения вида `FIC-1025`, `P-201A`, `D228`, `УПП-2` по всей книге и анализирует заголовки таблиц. Столбцы `Тег`, `Наименование параметра`, `Узел`, `Оборудование` и похожие предлагаются для обезличивания автоматически. Пользователь проверяет или меняет список столбцов один раз; все уникальные текстовые названия под ними извлекаются без ручного ввода. Числовые значения, формулы, даты и единицы измерения из остальных столбцов не изменяются.
...
| `outputs/anonymized/` | Обезличенные документы, ключи и восстановленные копии |
