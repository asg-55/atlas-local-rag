...
Если внешний сервис преобразовал обезличенный Office-файл в `CSV`, `TXT`, `JSON` или `MD`, Atlas может применить исходный ключ к такому текстовому результату. Расширение текстового результата не обязано совпадать с расширением исходного документа: восстанавливаются все сохранившиеся метки Atlas, а имя скачиваемого файла формируется из имени фактически загруженного результата.
