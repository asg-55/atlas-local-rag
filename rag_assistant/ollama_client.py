...
Поисковый запрос:"""
    ...
    def answer(
        ...
        mode_instructions = {
            ...
            "Подробный ответ": "Дай прямой естественный ответ и добавь только полезные для задачи детали. Не создавай формальные разделы «Параметры ответа», «Условия» и «Оговорки».",
        ...
        if answer_mode == "Обсуждение кода":
            improvement_advice = bool(
                re.search(
                    r"\b(?:как (?:можно )?(?:усилить|улучшить|доработать)|"
                    r"что (?:можно )?(?:усилить|улучшить|доработать)|какие улучшения)\b",
                    question.casefold(),
                )
            )
            improvement_rule = (
                "\n6. Пользователь просит сначала оценить улучшения. Дай приоритетный список "
                "изменений, объясни пользу и риски и предложи выбрать вариант. В этом ответе "
                "не показывай полный или изменённый код и не помещай текст в блоки кода."
...
Ответ:"""
            ...
            if improvement_advice and re.search(r"```", answer):
                correction_prompt = f"""Перепиши ответ на русском языке как короткий приоритетный список рекомендаций.

Правила:
1. Сохрани полезные объяснения, пользу и риски улучшений.
2. Полностью убери весь программный код, псевдокод и Markdown-блоки с тройными обратными кавычками.
3. Не предлагай готовую реализацию; заверши предложением выбрать улучшение для следующего шага.

Исходный ответ:
{answer}

Исправленный ответ без кода:"""
                try:
                    answer = self.generate(
                        correction_prompt,
                        temperature=0.0,
                        num_predict=min(num_predict, 1200),
                        model=model,
                        top_p=top_p,
                        num_ctx=num_ctx,
                        think=False,
                ...
                if not answer:
                    answer = (
                        "Предлагаю сначала выбрать одно улучшение: обработку ошибок, "
                        "журналирование или проверку входных данных. После выбора я покажу "
...
6. Не описывай устройство собственного ответа и не добавляй разделы «Параметры ответа», «Условия» или «Оговорки».
7. Если последний вопрос меняет тему, отвечай только на него: не упоминай прежние вложения, документы и задачи, если пользователь явно к ним не обращается.
{extra_rule}
...
11. Если в источниках полностью отсутствует ответ на главный вопрос, ответь одной короткой фразой «В источниках это не указано» с уточнением искомого поля. Не добавляй таблицу, пересказ источника и нерелевантные сведения.
{fallback}
{extra_rule}
